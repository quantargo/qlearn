...
tutorial:
  id: "course-platform-new#introduction#00-01"
  version: 0.1
output: 
  learnr::tutorial:
    progressive: false
...
library(tidyverse)
learnr::tutorial_options(exercise.timelimit = 60, exercise.checker = gradethis::grade_learnr)
...
```{r exercise-03-03-00, error = TRUE}
babynames
```
...
```{r exercise-03-03-01, exercise = TRUE, exercise.setup = "bnames"}

```

```{r exercise-03-03-01-hint, eval = FALSE}
library(baby...)
```

```{r exercise-03-03-01-solution}
library(babynames)
```
...
```{r exercise-03-03-02, exercise = TRUE, exercise.setup = "bnames"}

```

```{r exercise-03-03-02-solution}
library(tibble)
babynames
```
...
When you load tidyverse it instructs R to load _the most commonly used_ tidyverse packages. These are:
...
```{r exercise-03-03-03, exercise = TRUE, exercise.setup = "bnames"}

```

```{r exercise-03-03-03-solution}
library(tidyverse)
```

## Quiz

```{r exercise-03-03-04, echo = FALSE}
learnr::question('Which package is not loaded by `tidyverse`?',
  learnr::answer("ggplot2"),
  learnr::answer("dplyr"),
  learnr::answer("tibble"),
  learnr::answer("babynames", correct = TRUE, message = "Now that you are familiar with the data set, and have loaded the necessary packages, let's explore the data."),
  allow_retry = TRUE
)
```
...
This tutorial also introduced the babynames dataset. In the next tutorial, you will use this data set to plot the populrity of _your_ name over time. Along the way, you will learn how to filter and subset data sets in R.
